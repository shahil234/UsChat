...
const FriendList = () => {
  ...
  if(friendListData?.data?.length === 0 || !friendListData?.data){
    return <div className="h-screen flex items-center justify-center text-center text-xl">You have no friends yet</div>
  }
  return (
...
const FriendCard = ({ username, userId, avatar, setRefetchFriendList }) => {
  ...
  return (
    <div className="col-span-12 md:col-span-6 lg:col-span-4 xl:col-span-3 flex items-center gap-4">
      <div>
        <img className="h-32 w-32 rounded-full object-fit" src={avatar || dummyProfile} alt="" />
      </div>
      <div className="flex flex-col gap-2">
        <span className="font-medium block text-center">{username}</span>
        <div>
          <Button onClick={unFriendUser} disabled={deleting} variant="destructive">Remove</Button>
        </div>
      </div>
    </div>
  );
